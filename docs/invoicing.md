...
- `BusinessInvoiceSettings`: هویت رسمی، اطلاعات بانکی، شرایط پرداخت، لوگو، مهر، امضا و ظاهر
  پیش‌فرض هر کسب‌وکار.
- `SalesInvoice`: اطلاعات خریدار، وضعیت پرداخت، ارز/واحد نمایش، تفکیک همه اجزای مالی، snapshotها
  و امضای اختیاری خریدار.
...
فاکتور مشتری عادی ledger entry ایجاد نمی‌کند. فاکتور همکار فقط از معامله/توافق تأییدشده ساخته
می‌شود تا invariant «یک رویداد تجاری، یک Trade، دو ورودی دفتر و یک Invoice» حفظ شود.

مانده قبلی برای فاکتور همکار از entry همان معامله snapshot می‌شود. پیش‌فرض صرفاً اطلاع‌رسانی است
و در مبلغ قابل پرداخت جمع نمی‌شود. این وضعیت و تصمیم inclusion صریحاً روی سند چاپ می‌شوند. صدور،
چاپ، PDF، PNG، تکثیر یا ابطال فاکتور هیچ ledger entry جدیدی ایجاد نمی‌کند.
