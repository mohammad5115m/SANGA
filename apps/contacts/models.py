...
class Contact(models.Model):
    """A private business contact — a colleague («همکار») in the stone trade.

    Only stone sellers and traders have accounts here, so a contact has no
    relationship type: everyone in this list is a colleague, sometimes buying and
    sometimes selling. A contact belongs to exactly one owning ``business`` and is
    never visible to any other business. ``linked_business`` is an optional,
    privacy-safe pointer to a real platform business — it never exposes the linked
    business's own private contacts or financial records.
    """
    ...
    linked_business = models.ForeignKey(
        ...
        help_text="اتصال اختیاری به یک همکار در سنگا",
    )
