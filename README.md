...
4. فاکتور واحد برای مشتری، همکار ثبت‌شده و همکار محلی
5. تأیید دوطرفه همان نسخهٔ فاکتور و ثبت امضاها
6. تسویهٔ نقدی، اعتباری، چک یا ترکیبی و دفتر حساب هر همکار
7. کاتالوگ‌های زنده
8. اطلاعات قیمت و موجودی با تاریخ اعتبار

## آنچه سنگا نیست

درگاه پرداخت، انبارداری، ERP، حسابداری رسمی، لجستیک، مزایده معکوس،
تابلوی تقاضای عمومی، یا CRM سازمانی. CRM سبک سنگا فقط پرونده مشتری، یادداشت و
پیگیری‌های فروش را کنار درخواست‌ها نگه می‌دارد. پول بیرون از سنگا جابه‌جا می‌شود
و بعداً ثبت می‌شود.
...
ورود آزمایشی: هر زمان `SMS_PROVIDER=console` باشد، OTP در صفحه تأیید نمایش
داده می‌شود و در لاگ کانتینر `web` نیز چاپ می‌شود؛ این رفتار به `DEBUG` وابسته
نیست. در
...
apps/
  ...
  trading/      رکورد نهایی و تاریخچهٔ گردش‌های قدیمی
  invoicing/    مرکز فاکتور، تأیید، تسویه و چک
...
| [docs/trading.md](docs/trading.md) | توافق دوطرفه و معامله نهایی |
| [docs/invoicing.md](docs/invoicing.md) | معماری سند و خروجی فاکتور |
| [docs/invoice-first-workflows.md](docs/invoice-first-workflows.md) | چرخه‌های invoice-first، تسویه و انتشار |
...
- پیش‌نویس و فاکتور در انتظار تأیید هیچ اثر مالی ندارند.
- تأیید دوطرفهٔ یک نسخهٔ فاکتور، دو سند دفتر و یک رکورد نهایی معامله می‌سازد.
- فروش نهایی موجودی را خودکار کم نمی‌کند.
- هر فاکتور همکار دقیقاً یک‌بار در دفتر حساب ثبت می‌شود؛ فاکتور مشتری دفتر ندارد.
